-- P0: a provider who receives a lead (provider_notified) and replies directly
-- (without first tapping "Abrir solicitud" / provider_opened) was rejected with
-- invalid_lead_transition. Allow provider_replied from provider_notified so the
-- provider can contact the customer immediately; replying implicitly opens the lead.
...
begin
  ...
  if not (
    ...
    -- Provider may reply directly from provider_notified (implicitly opens the lead).
    (p_event_type = 'provider_replied' and v_last_event in ('provider_opened', 'customer_replied', 'provider_notified')) or
