...
export default async function AdminPage() {
  ...
  return <main className="mx-auto min-h-screen max-w-5xl px-5 py-10"><header className="mb-9 flex items-end justify-between gap-4"><div><span className="text-xs font-extrabold tracking-[.12em] text-bs-primary uppercase">BuenServ · Admin</span><h1 className="font-display mt-2 text-4xl font-extrabold tracking-[-.06em]">Cola de moderación</h1><p className="mt-2 text-sm text-bs-muted">Rol actual: {actor.role}</p></div><span className="rounded-full bg-bs-mint px-3 py-2 text-sm font-extrabold text-bs-primary-dark">{providers.length} pendientes</span></header><ModerationQueue initialProviders={providers}/></main>;
}
