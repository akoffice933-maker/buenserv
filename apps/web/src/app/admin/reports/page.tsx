...
export default async function AdminReportsPage() {
  ...
  const {data} = await createAdminClient().from('reports').select('id,reason,details,status,created_at,providers(slug,profiles!providers_profile_id_fkey(display_name))').in('status', ['open', 'reviewing']).order('created_at');
