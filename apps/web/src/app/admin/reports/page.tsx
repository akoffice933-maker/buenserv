...
const {data} = await createAdminClient().from('reports').select('id,reason,details,status,created_at,providers!reports_provider_id_fkey(slug,profiles!providers_profile_id_fkey(display_name)),profiles!reports_reporter_profile_id_fkey(display_name,telegram_user_id)').in('status', ['open', 'reviewing']).order('created_at');
