...
export default async function AdminSupportPage() {
  ...
  const {data} = await createAdminClient().from('support_requests').select('id,details,status,created_at,profiles(display_name,telegram_user_id)').in('status', ['open', 'reviewing']).order('created_at');
