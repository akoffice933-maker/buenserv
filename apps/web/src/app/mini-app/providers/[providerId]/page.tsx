...
type ProviderData = {
  id: string; slug: string; bio?: string | null; photoPath?: string | null; displayName: string;
...
export default function ProviderDetailPage() {
  ...
  const firstBarrio = one(p.barrios[0]?.barrios);
  const headerTitle = [firstCat ? (locale === 'ru' ? firstCat.name_ru : locale === 'en' ? firstCat.name_en : firstCat.name_es) : null, firstBarrio ? barrioName(firstBarrio) : null].filter(Boolean).join(' · ');

  return (
    <MiniAppShell title={headerTitle} showBack showBottomNav={false}>
      <div className="space-y-5 pb-8 -mt-1">
        <div className="bg-white rounded-[24px] p-5 border border-[#DCE4DE]/80 bs-card-shadow relative overflow-hidden">
          ...
          <div className="flex items-start gap-4 pt-1">
            <div className="w-20 h-20 rounded-[18px] bg-gradient-to-br from-[#0FA37F] to-[#08735A] text-white text-2xl font-black flex items-center justify-center shrink-0">{p.displayName.slice(0, 2).toUpperCase()}</div>
            <div className="flex-1 min-w-0">
              <h2 className="text-[22px] font-extrabold text-[#1A1F1D] tracking-tight leading-snug">{p.displayName}</h2>
              <p className="text-[13px] text-[#66706B] mt-0.5">{headerTitle}</p>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 mt-1.5 rounded-full bg-[#EAF7F1] text-[#0FA37F] text-[12px] font-bold border border-[#0FA37F]/30"><ShieldCheck className="w-3.5 h-3.5" />{t('status_verified')}</div>
            </div>
          </div>
        </div>
        ...
        {p.bio && (
          <div className="bg-white rounded-[20px] p-5 border border-[#DCE4DE]/80 bs-card-shadow space-y-2">
            <h3 className="text-[16px] font-bold text-[#1A1F1D]">{t('profile_about')}</h3>
            <p className="text-[14px] text-[#3d4742] leading-relaxed m-0">{p.bio}</p>
          </div>
        )}
            ...
            {p.categories.map((c) => (
              <div key={c.categoryId} className="py-3 first:pt-0 last:pb-0 space-y-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  {c.priceFromArs ? <span className="text-[14px] font-extrabold text-[#0FA37F] shrink-0">{t('price_from')} {formatPrice(c.priceFromArs, locale)} ARS</span> : null}
                </div>
              </div>
            ))}
          </div>
        ...
        <PrimaryButton onClick={() => router.push(`/mini-app/contact/${p.id}`)}>{t('btn_write_provider')}</PrimaryButton>
