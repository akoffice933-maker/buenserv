...
export async function GET() {
  ...
  const {data, error} = await createAdminClient().from('audit_events').select('id,action,entity_type,entity_id,metadata,created_at,profiles(display_name)').order('created_at', {ascending: false}).limit(100);
