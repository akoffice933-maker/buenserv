...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase.from('providers').select('id,slug,photo_path,rating,reviews_count,accepts_usdt,profiles!providers_profile_id_fkey(display_name),provider_categories(price_from_ars,categories!provider_categories_category_id_fkey(slug)),provider_barrios(barrios!provider_barrios_barrio_id_fkey(slug,name_es,name_ru,name_en))').eq('status', 'approved').order('rating', {ascending: false}).limit(parsed.data.limit);
