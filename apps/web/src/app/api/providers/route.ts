...
import {PROVIDER_PUBLIC_SELECT} from '@/lib/supabase/selects';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase.from('providers').select(PROVIDER_PUBLIC_SELECT).eq('status', 'approved').order('rating', {ascending: false}).limit(parsed.data.limit);
