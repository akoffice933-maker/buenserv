...
import {BARRIO_SLUGS, barrioLabel} from '@/lib/barrios';
...
export function ProviderDirectory({category, locale}: {category: string; locale: string}) {
  ...
  if (state.status === 'loading') content = <div aria-live="polite" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[0, 1, 2].map(item => <div key={item} className="h-53 animate-pulse rounded-2xl border border-black/7 bg-white p-5"><div className="size-12 rounded-full bg-black/7"/><div className="mt-7 h-5 w-2/3 rounded bg-black/7"/><div className="mt-3 h-4 w-1/2 rounded bg-black/7"/></div>)}</div>;
    ...
    content = <section aria-label={t('results')} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{state.providers.map(provider => {
      ...
      const displayName = one(provider.profiles)?.display_name ?? provider.slug.replaceAll('-', ' ');
      return <Link key={provider.id} href={`/${locale}/profile/${provider.slug}`} className="rounded-2xl border border-black/8 bg-white p-5 shadow-[0_8px_24px_rgba(23,53,42,.05)] no-underline transition hover:-translate-y-1 hover:shadow-[0_14px_30px_rgba(23,53,42,.10)]"><div className="flex items-center gap-3"><div className="grid size-12 place-items-center rounded-full bg-bs-mint font-display text-lg font-extrabold text-bs-primary">{provider.slug.slice(0, 1).toUpperCase()}</div><div><h2 className="font-display text-xl font-extrabold">{displayName}</h2><p className="text-xs text-bs-muted">{barrioName}</p></div></div><p className="mt-5 text-sm"><span className="text-[#B86D00]">★★★★★</span> <b>{provider.rating.toFixed(1)}</b> · {provider.reviews_count} {t('reviews')}</p><div className="mt-4 flex items-center justify-between border-t border-black/7 pt-4"><span className="font-extrabold">{price ? currency.format(price) : t('askPrice')}</span>{provider.accepts_usdt && <span className="rounded-full bg-bs-mint px-2 py-1 text-[11px] font-extrabold text-bs-primary-dark">USDT ✓</span>}</div></Link>;
    })}</section>;
  }

  return <div className="mt-8"><div className="mb-5 flex flex-wrap items-center gap-3"><label className="text-sm font-bold"><span className="sr-only">{t('allBarrios')}</span><select value={barrio} onChange={event => setBarrio(event.target.value)} className="rounded-lg border border-black/12 bg-white px-3 py-2"><option value="">{t('allBarrios')}</option>{BARRIO_SLUGS.map(item => <option key={item} value={item}>{barrioLabel(item, locale)}</option>)}</select></label><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={usdt} onChange={event => setUsdt(event.target.checked)}/>{t('onlyUsdt')}</label></div>{content}</div>;
}
