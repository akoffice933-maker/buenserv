...
export function ReportProvider({providerId, botUsername}: {providerId: string; botUsername: string}) {
  ...
  if (!open && botUsername) return <a href={`https://t.me/${botUsername}?start=report_${providerId}`} className="mt-4 inline-block text-xs font-bold text-bs-muted underline">{t('open')}</a>;
