...
type Profile = DirectoryProvider & {bio?: string | null; profiles?: {display_name: string | null} | Array<{display_name: string | null}> | null; reviews?: Array<{rating: number; body: string; locale: string; created_at: string}>};

export function ProviderProfile({slug, locale, botUsername}: {slug: string; locale: string; botUsername: string}) {
  ...
  const profileRecord = Array.isArray(profile.profiles) ? profile.profiles[0] : profile.profiles;
